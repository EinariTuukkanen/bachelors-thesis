...
\begin{table}[H]
    \centering
    \captionof{table}{The average of the durations for finding the minimum for both Matrix Square Sum and Negative Entropy Functions combined and listed separately for each main method. Colored per column so that darker red is worse (slower) and darker green is better (faster).}
...
% While it is relatively clear that different parameters produce different results and performance, it is a really complex problem to choose the absolute best values for each parameter. Often picking a value for one parameter also effects the optimal values for other parameters and therefore we consider 
...
%   - low iteration count on comparison
% - domain limiter
%   - parameter
